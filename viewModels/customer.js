...
function smartJoin(arr,separator){//equals to join actually, if contents are legit.
...
module.exports = function(customerId){
  ...
  var orders = customer.getOrders().map(function(order){//grouping through ID.
    return {
  ...
  return {
    ...
    orders:customer.getOrders().map(function(order){//grouping through ID.
      return {
        orderNumber:order.orderNumber,
        date:order.date,
        status:order.status,
