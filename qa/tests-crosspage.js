...
suite('Cross-Page Test',function(){
    ...
    test('requesting a group rate quote from the hood river tour page should populate the referrer field',function(done){
        var referrer = 'http://localhost:9527/tours/hood-river';
    ...
    test('requesting a group rate from the oregon coast tour page should populate the referrer field',function(done){
        var referrer = 'http://localhost:9527/tours/oregon-coast';
        browser.visit(referrer,function(){
            browser.clickLink('.requestGroupRate',function(){
                assert(browser.field('referrer').value === referrer);
                done();
    ...
    test('visiting the "request group rate" directly should result in an empty referrer field', function(done){
        browser.visit('http://localhost:9527/tours/request-group-rate', function(){
